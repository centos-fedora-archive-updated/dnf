...
pkgs.append(rpm)
# Used for install remove tests_list if requirement TestB is handled properly.

rpm = SimpleRpmBuild('TestA', '1.0.0', '2', ['noarch'])
rpm.add_requires('TestB')
pkgs.append(rpm)
# Used for upgrade tests_list
...
pkgs.append(rpm)
# Used for install remove tests_list as a negative control - should not be installed if you install TestA.
...
pkgs.append(rpm)
# Used for install remove tests_list if requirement with = version is handled properly.
...
pkgs.append(rpm)
# Used for install remove tests_list if requirement with >= version and multiple requirements are handled properly.
...
pkgs.append(rpm)
# Used for upgrade tests_list - negative control
...
pkgs.append(rpm)
# Used for remove tests_list. Multiple packages require TestM therefore TestM cannot be removed if package is required by
# other package.

rpm = SimpleRpmBuild('TestK', '1.0.0', '2', ['noarch'])
rpm.add_requires('TestJ >= 1.0.0-3')
pkgs.append(rpm)
# Used for install tests_list - negative control due to requirement
...
pkgs.append(rpm)
# Used in upgrade, downgrade, upgrade-to tests_list
